...
public class IsInCircRNA1_1 {
	...
	public int isInCircRNA1_1(int len_str,String str,String circ_range_seq,String linear_range) {
		 for (int k = 0; k < window_unit.length; k++) {
				...
				for (int j = 0; j <= trial; j++) {
					...
					String seq = str.substring(len_str-j*window_step-window_size,len_str-j*window_step);
					int locus = circ_range_seq.lastIndexOf(seq);
					int locus2 = linear_range.lastIndexOf(seq);	
					if (locus >= 0) {
				...
				if (len_str % window_unit[k] != 0) {
					trial++;
					String seq = str.substring(0,window_size);
					int locus = circ_range_seq.lastIndexOf(seq);
					int locus2 = linear_range.lastIndexOf(seq);
					if (locus >= 0) {
						locusListTem.add(locus);
